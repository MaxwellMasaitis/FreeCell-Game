...
public class AppView extends JFrame{
	...
	public AppView(FreeCellGame inputGame) {
		...
		statsButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				statString = String.format("Games Played: %d\nGames Won: %d\nWin Rate: %.2f%%\nCurrent Moves: %d\nFastest Win: %d", game.getGamesPlayed(), game.getWinCount(), game.getWinRate() * 100, game.getMoveCount(), game.getLeastMoves());
				...
				if (! game.makeMove(fromPanel.getCell(), panel.getCell())) {
					...
					int reply = JOptionPane.showConfirmDialog(AppView.this, "You win! Do you want to play again?",
						...
						if (reply == JOptionPane.YES_OPTION) {
					        	...
					        	statString = "Your final stats were:\n" + String.format("Games Played: %d\nGames Won: %d\nWin Rate: %.2f%%\nCurrent Moves: %d\nFastest Win: %d", game.getGamesPlayed(), game.getWinCount(), game.getWinRate() * 100, game.getMoveCount(), game.getLeastMoves());
					        	JOptionPane.showMessageDialog(AppView.this, statString);
					        	System.exit(0);}	
					}
					else if (game.loser()) {
